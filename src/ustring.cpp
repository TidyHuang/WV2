...
UString::UString(const UString &b)
{
	...
	{
		UChar *d = allocateChars( length );
		memcpy(d, b.data(), length * sizeof(UChar));
		rep = Rep::create(d, length);
	}
	else
	{
...
char *UString::ascii() const
{
	if (buf->staticBuffer)
		delete[] buf->staticBuffer;
	buf->staticBuffer = new char[length()+1];
	for(int i = 0; i < length(); i++)
	  buf->staticBuffer[i] = data()[i].low();
	buf->staticBuffer[length()] = '\0';

  return buf->staticBuffer;
}
...
UString &UString::operator=(const UString &str)
{
  ...
  {
	  UChar *d = allocateChars( length );
	  memcpy(d, str.data(), length * sizeof(UChar));
	  rep = Rep::create(d, length);
  }
  else
  {
...
UConstString::UConstString( UChar* data, unsigned int length ) : UString( data, length )
{
}
